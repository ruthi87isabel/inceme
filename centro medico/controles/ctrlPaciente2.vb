...
#Region "Private Sub CargarPaciente(ByVal id As Integer, ByVal IsReturnPressed As Boolean)"
    Private Sub CargarPaciente(ByVal id As Nullable(Of Integer), ByVal codigoPropio As String, ByVal IsReturnPressed As Boolean)
        Try
            ...
            If id.HasValue Then
                Paciente = (From p In Globales.Context.PACIENTEs Where p.CPACIENTE = id And (Not p.Eliminado.HasValue Or p.Eliminado = False) Select p).SingleOrDefault()
            Else
                Paciente = (From p In Globales.Context.PACIENTEs Where p.CODIGOPROPIO = codigoPropio And (Not p.Eliminado.HasValue Or p.Eliminado = False) Select p).SingleOrDefault()
            End If
...
#Region " Private Sub Muestra()"
    Private Sub Muestra()
        ...
        'If (Not Paciente.DNI Is Nothing) Then
        ...
        'lblBeneficiario.Visible = (Paciente.BENEFICIARIOCODIGOSOCIO > 0)
